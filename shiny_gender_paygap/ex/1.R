...
y <- y[, lapply(.SD, mean, na.rm = TRUE), section_desc,  .SDcols = cols][order(section_desc)]
tbl <- BasicTable$new()
tbl$addData(y, 
        firstColumnAsRowHeaders = TRUE, 
        explicitColumnHeaders = c('Section', mtcs$description),
...
y <- dts[!is.na(section_code)]
y <- dcast(y, section_desc~datefield, value.var = 'DMH', fun.aggregate = mean, na.rm = TRUE)
...
get_tbl_sic <- function(x){
  y <- dts[section_code == x][!is.na(sic_code)]
  y <- dcast(y, sic_desc~datefield, value.var = 'DMH', fun.aggregate = mean, na.rm = TRUE)
  y[, var := 100 * (`2018` / `2017` - 1)]
  tbl <- BasicTable$new()
  tbl$addData(y, 
          firstColumnAsRowHeaders = TRUE, 
          explicitColumnHeaders = c(paste('SIC of Section:', sections[which(names(sections) == 'A')]), '2017', '2018', 'Variation'),
          columnFormats = rep(list("%.2f"), ncol(y))
  )
  cells <- tbl$findCells(columnNumbers = 4, maxValue = 0, includeNull = FALSE, includeNA = FALSE)
  tbl$setStyling(cells = cells, declarations = list("background-color" = "#38EB6B", "color" = "#9C0006"))
...
get_tbl_rgn('E12000008')
get_tbl_rgn(lcn[type == 'RGN' & name == 'Wales', location_id])
...
y <- dts[datefield == 2018, c('section_desc', cols), with = FALSE]
# y <- lcn[type == 'LAD', .(location_id, name)][y, on = c(location_id = 'LAD')][, location_id := NULL][order(name)]
y <- y[, lapply(.SD, mean), section_desc, .SDcols = cols][order(section_desc)]


dt <- datatable( 
        y, 
        rownames = FALSE ,
        colnames = c('Section' = 'section_desc'),
        selection = 'none',
        class = 'cell-border nowrap',
        extensions = c('Buttons', 'FixedColumns', 'Scroller'),
        options = list(
